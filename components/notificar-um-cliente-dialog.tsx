...
export type ClienteAlvoNotificacao = { id: number; nome: string; info?: string }

interface NotificarUmClienteDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  cliente: ClienteAlvoNotificacao | null
  onEnviado?: () => void
}

export function NotificarUmClienteDialog({
  open,
  onOpenChange,
  cliente,
  onEnviado,
}: NotificarUmClienteDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Mensagem enviada para ${cliente.nome}.`,
      })
      onOpenChange(false)
      onEnviado?.()
    } catch (error: unknown) {
      ...
      {cliente ? (
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              A notificação será enviada para o usuário vinculado a{" "}
              <span className="font-medium text-foreground">{cliente.nome}</span>
              {cliente.info ? ` (${cliente.info})` : ""}, quando existir.
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="cli-notif-ind-titulo">Título *</Label>
              <Input
                id="cli-notif-ind-titulo"
                value={titulo}
            ...
            <div className="space-y-2">
              <Label htmlFor="cli-notif-ind-msg">Mensagem *</Label>
              <Textarea
                id="cli-notif-ind-msg"
                value={mensagem}
                onChange={(e) => setMensagem(e.target.value)}
                placeholder="Texto que o cliente verá no painel"
                rows={4}
                required
              />
            </div>
