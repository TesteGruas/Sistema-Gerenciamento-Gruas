...
export const obrasArquivosApi = {
  ...
  async obter(obraId: number, arquivoId: number): Promise<ArquivoResponse> {
    const response = await api.get(`/obras/${obraId}/documentos/${arquivoId}`)
    return response.data
  },
  ...
  async upload(dados: ArquivoCreate): Promise<ArquivoResponse> {
    ...
    const response = await api.post(`/obras/${dados.obra_id}/documentos`, formData, {
      headers: {
  ...
  async uploadMultiple(obraId: number, arquivos: File[], categoria?: string): Promise<UploadMultipleResponse> {
    ...
    const response = await api.post(`/obras/${obraId}/documentos/upload-multiple`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    })
  ...
  async atualizar(obraId: number, arquivoId: number, dados: ArquivoUpdate): Promise<ArquivoResponse> {
    const response = await api.put(`/obras/${obraId}/documentos/${arquivoId}`, dados)
    return response.data
  },

  // Excluir arquivo
  async excluir(arquivoId: number): Promise<ArquivoResponse> {
    const response = await api.delete(`/obras/documentos/${arquivoId}`)
    return response.data
  },

  // Baixar arquivo
  async baixar(obraId: number, arquivoId: number): Promise<void> {
    const response = await api.get(`/obras/${obraId}/documentos/${arquivoId}/download`)
