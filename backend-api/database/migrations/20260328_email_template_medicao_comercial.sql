-- Modelo comercial no e-mail (como envio manual): assunto MEDIÇÃO N - MÊS/ANO - OBRA - GRUA,
-- bloco comercial (saudação, texto, resumo NF + locação) e variáveis de assunto.

UPDATE email_templates
SET
  assunto = 'MEDIÇÃO {{numero}} - {{periodo_assunto}} - {{obra_nome_assunto}} - {{grua_nome_assunto}}',
  html_template = $MED$
<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
$MED$,
  variaveis = '["numero", "periodo", "periodo_assunto", "obra_nome_assunto", "grua_nome_assunto", "valor_total", "valor_mensal_bruto", "valor_aditivos", "valor_custos_extras", "valor_descontos", "grua_nome", "obra_nome", "cliente_nome", "data_medicao", "link_pdf", "empresa", "bloco_comercial_html", "historico_status_html", "documentos_resumo_html"]'::jsonb,
  updated_at = NOW()
