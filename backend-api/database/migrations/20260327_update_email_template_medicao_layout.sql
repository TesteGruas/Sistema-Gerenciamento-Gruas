-- Atualiza o HTML do template medicao_enviada (layout alinhado à tela de detalhes da medição).
-- Variáveis: escalares + historico_status_html + documentos_resumo_html (HTML gerado no backend).
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden;">
          <tr>
            <td style="padding:20px 24px;border-bottom:2px solid #2563eb;text-align:center;">
              ...
              <p style="margin:10px 0 0;font-size:14px;color:#4b5563;">Nova medição disponível para análise</p>
            </td>
          </tr>
          <tr>
            <td style="padding:20px 24px;">
              <p style="margin:0 0 16px;font-size:14px;line-height:1.5;color:#374151;">Olá, <strong>{{cliente_nome}}</strong>. Seguem os dados da medição <strong>nº {{numero}}</strong> ({{periodo}}).</p>
...
$MED$,
  variaveis = '["numero", "periodo", "valor_total", "valor_mensal_bruto", "valor_aditivos", "valor_custos_extras", "valor_descontos", "grua_nome", "obra_nome", "cliente_nome", "data_medicao", "link_pdf", "empresa", "historico_status_html", "documentos_resumo_html"]'::jsonb,
  updated_at = NOW()
